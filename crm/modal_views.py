...
def assign_executor_modal(request, order_id):
    ...
    if request.method == 'POST':
        ...
        order_rate = request.POST.get('order_rate', 0)
        ...
        try:
            ...
            if not order.executors.filter(user=executor).exists():
                # Если ставка не указана, берем базовую ставку исполнителя
                if not order_rate or float(order_rate) <= 0:
                    order_rate = company_user.order_rate
                    
                # Если базовая ставка тоже не установлена, предлагаем ставку
                if not order_rate or float(order_rate) <= 0:
                    from .utils import suggest_order_rate
                    order_rate = suggest_order_rate(order, executor)
                
                OrderExecutor.objects.create(
                    order=order,
                    user=executor,
                    order_rate=order_rate,
                    payment_amount=order_rate,  # Сумма к оплате равна ставке за заказ
                    notes=notes,
                    status='assigned'
                )
                ...
                return JsonResponse({
                    ...
                    'message': f'Исполнитель {executor.get_full_name()} назначен. '
                             f'К оплате при завершении: {order_rate}₽'
                })
            else:
                return JsonResponse({
                    'success': False,
            ...
            return JsonResponse({
                'success': False,
                'error': 'Пользователь не принадлежит к компании'
            })
    
    # Получаем исполнителей компании
    ...
    # Предлагаем ставку на основе заказа
    from .utils import suggest_order_rate
    suggested_rate = suggest_order_rate(order)

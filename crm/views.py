...
from .utils import get_user_orders, can_user_access_order, calculate_user_commission, calculate_user_orders_payment
from .forms import *
...
def company_select(request):
    ...
    # Если у пользователя только одна компания, автоматически выбираем её
    if user_companies.count() == 1:
        company_user = user_companies.first()
        request.session['current_company_id'] = company_user.company.id
        return redirect('dashboard')
    
    # Если нет доступных компаний
    if user_companies.count() == 0:
        messages.error(request, 'У вас нет доступа ни к одной компании. Обратитесь к администратору.')
    ...
    if request.method == 'POST':
        ...
        if not company_id:
            messages.error(request, 'Пожалуйста, выберите компанию')
        else:
            try:
                company_user = user_companies.get(company_id=company_id)
                request.session['current_company_id'] = company_user.company.id
                
                # Логируем успешный выбор компании
                messages.success(request, f'Вы вошли в компанию "{company_user.company.name}"')
                ...
                messages.error(request, 'Выбранная компания не найдена или у вас нет к ней доступа')
    ...
    return render(request, 'auth/company_select.html', context)
...
def clients_list(request):
    ...
    for client in page_obj:
        total_amount = client.orders.aggregate(total=Sum('total_amount'))['total'] or 0
        clients_data.append({
            'client': client,
...
def employees_list(request):
    ...
    stats = {
        ...
        'executor_count': employees.filter(role='executor').count(),
        'total_salary': employees.aggregate(total=Sum('salary_rate'))['total'] or 0,
    }
...
def finances_overview(request):
    ...
    projected_income = Decimal(str(month_income)) * Decimal(str(days_in_month / days_passed)) if days_passed else month_income
    projected_expenses = Decimal(str(month_expenses)) * Decimal(str(days_in_month / days_passed)) if days_passed else month_expenses
...
def my_salary(request):
    ...
    current_total = company_user.salary_rate + orders_data['total_payment']
    ...
    current_pending_orders = OrderExecutor.objects.filter(
        user=user,
        order__company=company,
        status__in=['assigned', 'in_progress'],
        assigned_at__year=current_date.year,
        assigned_at__month=current_date.month
    ).count()
    
    # Недавние завершенные заказы (для исполнителей)
    recent_orders = None
    if company_user.role == 'executor':
        recent_orders = OrderExecutor.objects.filter(
            user=user,
            order__company=company,
            status='completed',
            completed_at__isnull=False
        ).select_related('order', 'order__client').order_by('-completed_at')[:5]
    
    # Средняя ставка за заказ в текущем месяце
    current_average_per_order = orders_data['average_per_order'] if orders_data['orders_count'] > 0 else company_user.order_rate
    
    context = {
        'salary_calculations': salary_calculations,
        'current_orders_payment': orders_data['total_payment'],
        'current_orders': orders_data['orders_count'],
        'current_average_per_order': current_average_per_order,
        'company_user': company_user,
        'current_total': current_total,
        'month_progress': month_progress,
        'days_left': days_left,
        'current_pending_orders': current_pending_orders,
        'current_bonuses': 0,  # Будет рассчитываться при создании SalaryCalculation
        'current_penalties': 0,
        'recent_orders': recent_orders,
    }
...
def salary_calculate(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if existing_calc:
                messages.warning(request, f'Зарплата за {month:02d}/{year} уже рассчитана')
                return redirect('my_salary')
            
            # Рассчитываем оплату за заказы
            orders_data = calculate_user_orders_payment(user, request.company, year, month)
            
            # Рассчитываем бонусы за количество заказов
            bonuses = 0
            if orders_data['orders_count'] > 0:
                bonus_amount = calculate_bonus_for_orders_count(
                    orders_data['orders_count'], 
                    company_user.order_rate
                )
                bonuses = bonus_amount * orders_data['orders_count']
            ...
            messages.success(
                request, 
                f'Зарплата {user.get_full_name()} за {month:02d}/{year} рассчитана: '
                f'{salary_calc.total_amount}₽ (оклад: {salary_calc.base_salary}₽, '
                f'за заказы: {orders_data["total_payment"]}₽, заказов: {orders_data["orders_count"]}'
            ...
            messages.error(request, 'Пользователь не принадлежит к компании')
        except Exception as e:
            messages.error(request, f'Ошибка при расчете зарплаты: {str(e)}')
...
def api_salary_calculate(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                'success': True,
                'total_payment': float(orders_data['total_payment']),
                'orders_count': orders_data['orders_count'],
                'average_per_order': float(orders_data['average_per_order'])
            })
